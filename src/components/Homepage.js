...
class Homepage extends Component {
  ...
  sortRun(run_of_cards) {
    // takes in an array of cards as parameter (specifically a run of cards)

    // selection sort performs fewer swaps than bubble sort
    //  alert(this._hand.length);
    let i, j, min_idx, len = run_of_cards.length;
    let temp;
    ...
    for (i = 0; i < len; i++) {
      ...
      min_idx = i;
      for (j = i + 1; j < len; j++)
        if (run_of_cards[j]._number < run_of_cards[min_idx]._number)
          min_idx = j;
      // swap the found minimum element with the first element
      temp = run_of_cards[i];
      run_of_cards[i] = run_of_cards[min_idx];
      run_of_cards[min_idx] = temp;
    }
  ...
  checkPhase(ff, fs, ft, sf, ss, st) {
    ...
    if (this.playerArr[this.currentPlayer]._phase == 1) {
      ...
      if (this.isASet(set1, this.playerArr[this.currentPlayer]._hand)) {
        ...
        if (this.isASet(set2, this.playerArr[this.currentPlayer]._hand)) {
    ...
    else // player is on phase 2
    {
      ...
      if (this.isASet(set1, this.playerArr[this.currentPlayer].hand)) {
        run2.push(sf);
        run2.push(ss);
        run2.push(st);

        if (this.isARun(run2, this.playerArr[this.currentPlayer]._hand)) {
          this.playerArr[this.currentPlayer].moveCardsToBoard1(set1, this.playerArr[this.currentPlayer]._hand);
  ...
  endOfTurn() {
    ...
    if (this.playerArr[this.currentPlayer].isSkipped) {
      if (this.currentPlayer === this.playerArr.length - 1)
        this.currentPlayer = 0;
      else
        this.currentPlayer++;
  ...
  indexcardHandler = event => {
    this.setState({
      index1: this.refs.firstfirst.value, index2: this.refs.secondfirst.value, index3: this.refs.firstsecond.value,
